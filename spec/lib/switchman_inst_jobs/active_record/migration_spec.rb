...
describe SwitchmanInstJobs::ActiveRecord::Migration do
  it "Has a copy of all delayed_engine migrations" do
    ...
    delayed_migrations = if Rails.version < "7.2"
                           ...
                           ActiveRecord::MigrationContext.new(
                             delayed_migrations_path,
                             ActiveRecord::SchemaMigration.new(ActiveRecord::Base.connection.pool)
                           ).migrations
                         end
    ...
    switchman_inst_jobs_migrations = if Rails.version < "7.2"
                                       ActiveRecord::MigrationContext.new(
